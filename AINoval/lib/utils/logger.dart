...
class AppLogger {
  ...
  static void init() {
    ...
    Logger.root.onRecord.listen((record) {
      ...
      if (!kDebugMode && (record.level == Level.FINEST || record.level == Level.FINER || record.level == Level.FINE)) {
          return;
      }
      ...
      final messageHeader = '$lvlColor$timestamp [${record.level.name}] [${record.loggerName}] $emoji $resetColor';
      ...
      if (record.error != null) {
        ...
        final stackTraceString = _formatStackTrace(record.stackTrace, filterAppCode: true, maxLines: 15);
  ...
  static String _getLogLevelColor(Level level) {
    if (level == Level.FINEST || level == Level.FINER || level == Level.FINE) return '\x1B[90m'; // 灰色 (Verbose/Debug)
    if (level == Level.CONFIG || level == Level.INFO) return '\x1B[34m'; // 蓝色 (Info/Config)
  ...
  /// 格式化并过滤堆栈信息
  static String _formatStackTrace(StackTrace? stackTrace, {int maxLines = 10, bool filterAppCode = true}) {
    ...
    for (final line in lines) {
      ...
      bool isFrameworkCode = trimmedLine.contains(flutterPackagePrefix) || trimmedLine.startsWith(dartPrefix);

      // 如果开启过滤，只保留应用代码；否则不过滤
      // 同时，排除纯dart:前缀和flutter框架内部调用（除非没有应用代码帧时酌情显示）
      if (!filterAppCode || isAppCode || (!isFrameworkCode && !trimmedLine.startsWith('#'))) { // 也包含一些非 package 的项目内部调用格式
        // 尝试保持可点击的格式
    ...
    if (formattedLines.isEmpty && lines.isNotEmpty) {
       formattedLines.addAll(lines.take(maxLines).map((l) => '  ${l.trim()}').where((l) => l.length > 2));
    }
