...
class NovelRepositoryImpl implements NovelRepository {
  ...
  Future<Novel> fetchNovel(String id) async {
    ...
    try {
      final data = await _apiClient.getNovelById(id);
  ...
  Future<Scene> fetchSceneContent(
    ...
    try {
      ...
      try {
        final data = await _apiClient.getScenesByChapter(novelId, chapterId);
  ...
  DateTime _parseDateTime(dynamic dateTimeValue) {
    if (dateTimeValue is String) {
      ...
      try {
        ...
        AppLogger.e(
            'Services/api_service/repositories/impl/novel_repository_impl',
            '解析LocalDateTime失败, 值: $dateTimeValue',
            e);
        // 解析失败时返回当前时间
        return DateTime.now();
      }
      ...
      try {
        ...
        AppLogger.e(
            'Services/api_service/repositories/impl/novel_repository_impl',
            '解析Instant时间戳失败, 值: $dateTimeValue',
            e);
        return DateTime.now();
      }
      ...
      AppLogger.i(
          'Services/api_service/repositories/impl/novel_repository_impl',
          '未知的日期时间格式: $dateTimeValue (${dateTimeValue.runtimeType})');
      return DateTime.now();
    }
  }
  ...
  Future<Scene> updateSceneContent(String novelId, String actId,
    ...
    try {
      ...
      try {
        ...
        final data = await _apiClient.createScene(sceneJson);
        return _convertToSceneModel(data);
      } catch (e2) {
        AppLogger.e(
            'Services/api_service/repositories/impl/novel_repository_impl',
