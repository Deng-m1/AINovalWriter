...
class ChatRepositoryImpl implements ChatRepository {
  ...
  Stream<ChatSession> fetchUserSessions(String userId) {
    ...
    try {
       return apiClient.listAiChatUserSessionsStream(userId);
        ...
        return Stream.error(ApiExceptionHelper.fromException(e, '发起获取用户会话流失败'), stackTrace);
    }
  }
  ...
  Future<ChatSession> createSession({
    ...
    try {
      ...
      AppLogger.e('ChatRepositoryImpl',
          '创建会话失败: userId=$userId, novelId=$novelId',
          e,
          stackTrace);
       // 使用辅助方法处理错误
  ...
  Future<ChatSession> getSession(String userId, String sessionId) async {
    try {
       AppLogger.i('ChatRepositoryImpl',
           '获取会话: userId=$userId, sessionId=$sessionId');
       ...
       return session;
        ...
        AppLogger.e('ChatRepositoryImpl',
            '获取会话失败: userId=$userId, sessionId=$sessionId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
  ...
  Future<ChatSession> updateSession({
    ...
    try {
       ...
       return updatedSession;
        ...
        AppLogger.e('ChatRepositoryImpl',
            '更新会话失败: userId=$userId, sessionId=$sessionId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
  ...
  Future<void> deleteSession(String userId, String sessionId) async {
    try {
       AppLogger.i('ChatRepositoryImpl',
           '删除会话: userId=$userId, sessionId=$sessionId');
       await apiClient.deleteAiChatSession(userId, sessionId);
       AppLogger.i('ChatRepositoryImpl',
           '删除会话成功: sessionId=$sessionId');
    } catch (e, stackTrace) {
        AppLogger.e('ChatRepositoryImpl',
            '删除会话失败: userId=$userId, sessionId=$sessionId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
  ...
  Future<ChatMessage> sendMessage({
    ...
    try {
      ...
      AppLogger.e('ChatRepositoryImpl',
          '发送消息失败: userId=$userId, sessionId=$sessionId',
          e,
          stackTrace);
       // 使用辅助方法处理错误
    ...
    // 直接返回 ApiClient 的流
     try {
        return apiClient.streamAiChatMessage(
         ...
         return Stream.error(ApiExceptionHelper.fromException(e, '发起流式消息请求失败'), stackTrace);
     }
  }
  ...
  Stream<ChatMessage> getMessageHistory(
    ...
    // 直接返回 ApiClient 的流
     try {
        return apiClient.getAiChatMessageHistoryStream(userId, sessionId, limit: limit);
     } catch (e, stackTrace) {
         AppLogger.e('ChatRepositoryImpl', '发起获取消息历史流请求时出错 [同步]', e, stackTrace);
         return Stream.error(ApiExceptionHelper.fromException(e, '发起获取消息历史流失败'), stackTrace);
     }
  }
  ...
  Future<ChatMessage> getMessage(String userId, String messageId) async {
    try {
       AppLogger.i('ChatRepositoryImpl',
           '获取消息: userId=$userId, messageId=$messageId');
       ...
       return message;
        ...
        AppLogger.e('ChatRepositoryImpl',
            '获取消息失败: userId=$userId, messageId=$messageId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
  ...
  Future<void> deleteMessage(String userId, String messageId) async {
    try {
       AppLogger.i('ChatRepositoryImpl',
           '删除消息: userId=$userId, messageId=$messageId');
       await apiClient.deleteAiChatMessage(userId, messageId);
       AppLogger.i('ChatRepositoryImpl',
           '删除消息成功: messageId=$messageId');
    } catch (e, stackTrace) {
        AppLogger.e('ChatRepositoryImpl',
            '删除消息失败: userId=$userId, messageId=$messageId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
  ...
  Future<int> countSessionMessages(String sessionId) async {
    try {
       ...
       return count;
        ...
        AppLogger.e('ChatRepositoryImpl',
            '统计会话消息数量失败: sessionId=$sessionId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
  ...
  Future<int> countUserSessions(String userId) async {
    try {
       AppLogger.i('ChatRepositoryImpl',
           '统计用户会话数量: userId=$userId');
       final count = await apiClient.countAiChatUserSessions(userId);
       AppLogger.i('ChatRepositoryImpl',
           '统计用户会话数量成功: userId=$userId, count=$count');
       return count;
    } catch (e, stackTrace) {
        AppLogger.e('ChatRepositoryImpl',
            '统计用户会话数量失败: userId=$userId',
            e,
            stackTrace);
         // 使用辅助方法处理错误
