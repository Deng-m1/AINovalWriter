...
class ContextProvider {
  ...
  // 基于特定的检索词获取相关上下文
  Future<List<ContextItem>> searchRelevantContext(
      String novelId, String query) async {
    // 在第二周迭代中，我们使用模拟数据
    // 返回一些与查询相关的模拟上下文项
...
class CodexRepository {
  // 获取角色列表
  Future<List<CodexEntry>> getCharacters(String novelId,
      {int limit = 10}) async {
    // 模拟网络延迟
  ...
  // 获取地点列表
  Future<List<CodexEntry>> getLocations(String novelId,
      {int limit = 10}) async {
    // 模拟网络延迟
    await Future.delayed(const Duration(milliseconds: 500));
