...
class LocalStorageService {
  ...
  Future<List<ChatSession>> getSessionsToSync() async {
    ...
    for (final sessionId in syncList) {
      ...
      if (session != null) {
        ...
        AppLogger.w('LocalStorageService', 'getSessionsToSync: 未找到标记为同步的会话详情: $sessionId。考虑清除此标记。');
      }
    }
  ...
  Future<List<ChatMessage>?> getMessagesForSession(String sessionId) async {
    ...
    try {
      ...
      final messages = jsonList.map((json) => ChatMessage.fromJson(jsonDecode(json))).toList();
      messages.sort((a, b) => a.timestamp.compareTo(b.timestamp));
      return messages;
    } catch (e, stackTrace) {
      AppLogger.e('LocalStorageService', '解析会话 $sessionId 的消息失败', e, stackTrace);
      return null;
    }
  }

  /// 保存会话的消息列表（用于缓存历史记录）
  Future<void> saveMessagesForSession(String sessionId, List<ChatMessage> messages) async {
    final prefs = await _ensureInitialized();
    final key = 'chat_messages_$sessionId'; // Key for storing full history
  ...
  Future<void> addMessageToSessionHistory(String sessionId, ChatMessage message) async {
  ...
  Future<String> addPendingMessage({
    ...
    final pendingMessageData = {
      ...
      'content': content,
      'metadata': metadata,
      'timestamp': DateTime.now().toIso8601String(), // Store time added to queue
    };

    pendingList.add(jsonEncode(pendingMessageData));
    await prefs.setStringList(_pendingMessagesKey, pendingList);
    AppLogger.i('LocalStorageService', '添加待发送消息到队列: sessionId=$sessionId, localId=$localId');
