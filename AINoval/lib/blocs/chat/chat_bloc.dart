...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  Future<void> _onSelectChatSession(
    ...
    try {
      ...
      final messageStream =
          repository.getMessageHistory(_userId, event.sessionId);
    ...
    if (state is ChatSessionActive) {
      ...
      try {
        ...
        await _handleStreamedResponse(
            emit, placeholderMessage.id, event.content);
      } catch (e, stackTrace) {
  ...
  Future<void> _handleStreamedResponse(
      Emitter<ChatState> emit, String placeholderId, String userContent) async {
    // --- Initial state check ---
    if (state is! ChatSessionActive) {
          ...
          if (aiMessageIndex != -1) {
            final updatedStreamingMessage = ChatMessage(
              ...
              metadata: chunk.metadata ??
                  latestMessages[aiMessageIndex].metadata, // Merge metadata?
              actions: chunk.actions ??
                  latestMessages[aiMessageIndex].actions, // Merge actions?
            );
            ...
            AppLogger.w('ChatBloc',
                '_handleStreamedResponse: 未找到ID为 $placeholderId 的占位符进行流式更新');
  ...
  Future<void> _onUpdateChatModel(
    ...
    if (currentState is ChatSessionActive &&
      ...
      if (newSelectedModel == null) {
        // 添加日志记录找不到模型的具体ID
        AppLogger.w('ChatBloc',
            '_onUpdateChatModel: Model config with ID ${event.modelConfigId} not found in AiConfigBloc state.');
