...
class NovelImportBloc extends Bloc<NovelImportEvent, NovelImportState> {
  /// 创建小说导入Bloc
  NovelImportBloc({required this.novelRepository}) : super(NovelImportInitial()) {
  ...
  /// 处理导入小说文件事件
  Future<void> _onImportNovelFile(ImportNovelFile event, Emitter<NovelImportState> emit) async {
    ...
    try {
      ...
      emit(NovelImportInProgress(status: 'PROCESSING', message: '处理中...', jobId: jobId));
  ...
  /// 处理导入状态更新事件
  void _onImportStatusUpdate(ImportStatusUpdate event, Emitter<NovelImportState> emit) {
  ...
  /// 重置导入状态
  void _onResetImportState(ResetImportState event, Emitter<NovelImportState> emit) {
    _importStatusSubscription?.cancel();
    _importStatusSubscription = null;
    emit(NovelImportInitial());
  }
