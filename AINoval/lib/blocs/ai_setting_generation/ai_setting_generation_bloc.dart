...
class AISettingGenerationBloc extends Bloc<AISettingGenerationEvent, AISettingGenerationState> {
  ...
  Future<void> _onLoadInitialData(
    ...
    try {
      ...
      if (novel != null) {
        ...
        emit(AISettingGenerationDataLoaded(chapters: _loadedChapters));
      } else {
        AppLogger.e('AISettingGenerationBloc', 'Novel not found: ${event.novelId}');
        emit(AISettingGenerationFailure(error: '小说未找到', chapters: []));
      }
    } catch (e, stackTrace) {
      AppLogger.e('AISettingGenerationBloc', 'Error loading chapters for AI Panel', e, stackTrace);
      emit(AISettingGenerationFailure(error: '加载章节列表失败: ${e.toString()}', chapters: []));
    }
  }
  ...
  Future<void> _onGenerateSettingsRequested(
    ...
    try {
      ...
      emit(AISettingGenerationSuccess(generatedSettings: settings, chapters: currentChapters));
    } catch (e, stackTrace) {
      AppLogger.e('AISettingGenerationBloc', 'Error generating settings', e, stackTrace);
      emit(AISettingGenerationFailure(error: '生成设定失败: ${e.toString()}', chapters: currentChapters));
    }
  }
