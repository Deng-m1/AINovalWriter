...
class AiConfigBloc extends Bloc<AiConfigEvent, AiConfigState> {
  ...
  Future<void> _onLoadAiConfigs(
    ...
    try {
      final configs = await _repository.listConfigurations(userId: event.userId);
      emit(state.copyWith(
    ...
    emit(state.copyWith(modelsForProvider: [], selectedProviderForModels: event.provider));
    try {
      ...
      emit(state.copyWith(modelsForProvider: models, errorMessage: () => null));
         ...
         AppLogger.e('AiConfigBloc', '加载模型失败 for ${event.provider}', e, stackTrace);
         // Emit state clearing models and setting an error message
         emit(state.copyWith(modelsForProvider: [], errorMessage: () => '加载模型列表失败: ${e.toString()}'));
    }
  }

  Future<void> _onAddAiConfig(
      AddAiConfig event, Emitter<AiConfigState> emit) async {
    emit(state.copyWith(actionStatus: AiConfigActionStatus.loading, actionErrorMessage: () => null));
    try {
    ...
    try {
      ...
      if (index != -1) {
          ...
          emit(state.copyWith(actionStatus: AiConfigActionStatus.success, configs: currentConfigs));
      } else {
           // 如果找不到，最好还是重新加载
          emit(state.copyWith(actionStatus: AiConfigActionStatus.success));
    ...
    emit(state.copyWith(actionStatus: AiConfigActionStatus.loading, actionErrorMessage: () => null));
    try {
      await _repository.deleteConfiguration(userId: event.userId, configId: event.configId);
       // 从列表中移除
      final currentConfigs = List<UserAIModelConfigModel>.from(state.configs);
  ...
  Future<void> _onValidateAiConfig(
    ...
    try {
      ...
      final validatedConfig = await _repository.validateConfiguration(userId: event.userId, configId: event.configId);
      // 更新列表中的特定项
      final currentConfigs = List<UserAIModelConfigModel>.from(state.configs);
      final index = currentConfigs.indexWhere((c) => c.id == validatedConfig.id);
       if (index != -1) {
          currentConfigs[index] = validatedConfig;
          emit(state.copyWith(actionStatus: AiConfigActionStatus.success, configs: currentConfigs, loadingConfigId: null));
      } else {
          emit(state.copyWith(actionStatus: AiConfigActionStatus.success, loadingConfigId: null)); // Mark success but maybe reload
           add(LoadAiConfigs(userId: event.userId));
      }
    } catch (e, stackTrace) {
   ...
   Future<void> _onSetDefaultAiConfig(
    ...
    emit(state.copyWith(actionStatus: AiConfigActionStatus.loading, actionErrorMessage: () => null));
    try {
      final newDefaultConfig = await _repository.setDefaultConfiguration(userId: event.userId, configId: event.configId);
      // 重新加载整个列表以确保旧的默认值被取消
        emit(state.copyWith(actionStatus: AiConfigActionStatus.success));
      add(LoadAiConfigs(userId: event.userId));
    } catch (e, stackTrace) {
  ...
  void _onClearProviderModels(ClearProviderModels event, Emitter<AiConfigState> emit) {
