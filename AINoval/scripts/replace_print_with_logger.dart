...
const List<String> targetDirs = [
  'AINoval/lib/services',
  'AINoval/lib/screens',
  'AINoval/lib/blocs',
  'AINoval/lib/repositories',
  'AINoval/lib/ui',
];
...
void main() async {
  ...
  final libDir = Directory('${projectDir.path}/AINoval/lib');
  
  if (!await libDir.exists()) {
    print('错误: 找不到AINoval/lib目录，请确保在项目根目录运行此脚本');
    exit(1);
  }
  
  // 确保脚本目录存在
  final scriptsDir = Directory('${projectDir.path}/AINoval/scripts');
  ...
  if (!await loggerFile.exists()) {
    print('错误: 找不到日志工具类文件 (AINoval/lib/utils/logger.dart)');
  ...
  for (var targetDir in targetDirs) {
    final dir = Directory('${projectDir.path}/$targetDir');
    if (!await dir.exists()) {
