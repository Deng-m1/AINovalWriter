...
11. **保存选中大纲:** 前端将被选中的 **大纲标题** 和 **大纲内容** 发送给后端 (调用 API 4.2.3)。后端执行以下操作：
    ...
    * **设置场景标题:** 新 `Scene` 的标题默认使用 AI 生成并由用户选中的大纲标题 (`selectedOutlineTitle`)。
...
* 上下文的最大长度限制是多少？如何处理超长上下文？（截断？摘要的摘要？） (当前代码仅在日志中警告超长，未强制处理)
* **(已明确)** 保存大纲时，新创建的章节标题使用 AI 生成的标题 (`selectedOutlineTitle`)。新章节下的第一个场景的 **场景标题** 也默认使用 `selectedOutlineTitle`。
